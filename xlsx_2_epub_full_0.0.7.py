...
def generate_po_epub(data, epub_path, main_title, author_id, main_email):
    """生成符合新排版要求的EPUB"""
    # 安全处理输入参数（新增标题/副标题空置逻辑）
    safe_title = str(main_title).strip() if main_title is not None else ""
    safe_title = "" if safe_title == "无标题" else (safe_title if safe_title else "未知串号")
    
    safe_subtitle = str(main_email).strip() if main_email is not None else ""
    safe_subtitle = "" if safe_subtitle == "无名氏" else safe_subtitle
    ...
    # 构建标题区块（更新版）
    title_block = ['<div class="title-section">']
    if safe_title:
        title_block.append(f'<h1 class="main-title">{safe_title}</h1>')
    if safe_subtitle:
        title_block.append(f'<h2 class="subtitle">{safe_subtitle}</h2>')
